...
export class ConfluenceApiToken extends BaseApiToken {
  ...
  _encodeAuth(email, token, authType) {
    ...
    if (authType === "bearer") {
      ...
      logger.debug(
        `ConfluenceApiToken._encodeAuth: Plain bearer authType='${authType}', returning raw token (no encoding)`
      );
      return token;
      ...
      logger.debug(
        `ConfluenceApiToken._encodeAuth: Using Bearer+credentials encoding for authType='${authType}'`
      );
    } else {
