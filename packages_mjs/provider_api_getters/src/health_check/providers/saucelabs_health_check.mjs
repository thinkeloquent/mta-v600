 ...
 *
 * Run directly: node saucelabs_health_check.mjs
 *
 * Uses:
...
// Provider API getter
// ============================================================
import { SaucelabsApiToken } from '../../api_token/index.mjs';
...
export async function checkSaucelabsHealth() {
  console.log('='.repeat(60));
  console.log('SAUCELABS HEALTH CHECK');
  console.log('='.repeat(60));
  ...
  const username = apiKeyResult.username || 'me';
  const healthEndpoint = `/rest/v1/users/${username}`;
  console.log('\n[Request]');
  ...
  try {
    ...
    if (response.ok) {
      ...
      const usernameResp = data.username || 'N/A';
      ...
      console.log(`  Username: ${usernameResp}`);
      ...
      return {
        success: true,
        message: `Connected as ${usernameResp}`,
        data: {
          id: userId,
...
if (process.argv[1] === __filename) {
  console.log('\n');
  const result = await checkSaucelabsHealth();
  console.log('\n' + '='.repeat(60));
  console.log('RESULT');
  console.log('='.repeat(60));
