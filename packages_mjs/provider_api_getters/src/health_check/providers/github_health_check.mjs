#!/usr/bin/env node
/**
 * GitHub Health Check - Standalone debugging script with explicit 7-step pattern.
 *
 * Flow: YamlConfig -> ProviderConfig -> ProxyConfig -> AuthConfig -> RequestConfig -> Fetch -> Response
 *
 * Run directly: node github_health_check.mjs
 *
 * Uses:
...
// Provider API getter
// ============================================================
import { GithubApiToken } from '../../api_token/index.mjs';
...
/**
 * GitHub Health Check - Explicit 7-step building block pattern.
 *
 * Flow: YamlConfig -> ProviderConfig -> ProxyConfig -> AuthConfig -> RequestConfig -> Fetch -> Response
 *
 ...
 */
export async function checkGithubHealth(config = null) {
  // ============================================================
  ...
  const provider = new GithubApiToken(config);
...
if (process.argv[1] === __filename) {
  console.log('\n');
  console.log('='.repeat(60));
  console.log('GITHUB HEALTH CHECK - Explicit 7-Step Pattern');
  console.log('='.repeat(60));
