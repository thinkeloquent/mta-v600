...
```bash
curl -X GET "${JIRA_BASE_URL}/rest/api/2/myself" \
...
```bash
curl -X GET "${JIRA_BASE_URL}/rest/api/2/search?jql=project=MYPROJECT" \
  -H "Accept: application/json" \
  -u "${JIRA_EMAIL}:${JIRA_API_TOKEN}"
```
...
```bash
curl -X GET "${JIRA_BASE_URL}/rest/api/2/issue/{issueKey}" \
  -H "Accept: application/json" \
  -u "${JIRA_EMAIL}:${JIRA_API_TOKEN}"
```
...
```bash
curl -X GET "${JIRA_BASE_URL}/rest/api/2/project" \
  -H "Accept: application/json" \
  -u "${JIRA_EMAIL}:${JIRA_API_TOKEN}"
```
...
```bash
curl -X POST "${JIRA_BASE_URL}/rest/api/2/issue" \
  -H "Accept: application/json" \
...
```json
{
  "self": "https://your-company.atlassian.net/rest/api/2/user?accountId=...",
