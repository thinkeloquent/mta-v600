...
```bash
curl -X GET "${CONFLUENCE_BASE_URL}/wiki/rest/api/user/current" \
...
```bash
curl -X GET "${CONFLUENCE_BASE_URL}/wiki/rest/api/space" \
  -H "Accept: application/json" \
  -u "${CONFLUENCE_EMAIL}:${CONFLUENCE_API_TOKEN}"
```
...
```bash
curl -X GET "${CONFLUENCE_BASE_URL}/wiki/rest/api/space/{spaceKey}" \
  -H "Accept: application/json" \
  -u "${CONFLUENCE_EMAIL}:${CONFLUENCE_API_TOKEN}"
```
...
```bash
curl -X GET "${CONFLUENCE_BASE_URL}/wiki/rest/api/content/search?cql=type=page" \
  -H "Accept: application/json" \
  -u "${CONFLUENCE_EMAIL}:${CONFLUENCE_API_TOKEN}"
```
...
```bash
curl -X GET "${CONFLUENCE_BASE_URL}/wiki/rest/api/content/{pageId}?expand=body.storage" \
  -H "Accept: application/json" \
  -u "${CONFLUENCE_EMAIL}:${CONFLUENCE_API_TOKEN}"
```
...
```bash
curl -X POST "${CONFLUENCE_BASE_URL}/wiki/rest/api/content" \
  -H "Accept: application/json" \
