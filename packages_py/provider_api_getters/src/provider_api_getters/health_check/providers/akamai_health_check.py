...
Run directly: python -m provider_api_getters.health_check.providers.akamai_health_check
Or from project root: python packages_py/provider_api_getters/src/provider_api_getters/health_check/providers/akamai_health_check.py
...
if __name__ == "__main__":
    ...
    from provider_api_getters.api_token import AkamaiApiToken
else:
    # Relative import when used as module
    from ...api_token import AkamaiApiToken
...
async def check_akamai_health(config: dict = None) -> dict:
    """
    Check Akamai connectivity and return status.

    Note: Akamai uses EdgeGrid authentication. This health check
    attempts to verify credentials and connectivity.
    ...
    print("=" * 60)
    print("AKAMAI HEALTH CHECK")
    print("=" * 60)

    # Initialize provider from config
    provider = AkamaiApiToken(config)
    ...
    health_endpoint = "/diagnostic-tools/v2/ghost-locations/available"
    print(f"\n[Request]")
    ...
    async with client:
        try:
            ...
            if response["ok"]:
                ...
                print(f"\n[Akamai Info]")
                print(f"  Available ghost locations: {location_count}")

                return {
                    "success": True,
                    "message": f"Connected, {location_count} locations available",
                    "data": {
...
if __name__ == "__main__":
    ...
    print("\n")
    result = asyncio.run(check_akamai_health(static_config))
    print("\n" + "=" * 60)
    print("RESULT")
    print("=" * 60)
