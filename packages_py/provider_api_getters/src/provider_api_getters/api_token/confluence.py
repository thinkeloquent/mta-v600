...
class ConfluenceApiToken(BaseApiToken):
    ...
    def _encode_auth(self, email: str, token: str, auth_type: str) -> str:
        ...
        if auth_type == "bearer":
            ...
            logger.debug(
                f"ConfluenceApiToken._encode_auth: Plain bearer auth_type='{auth_type}', returning raw token (no encoding)"
            )
            return token
            ...
            logger.debug(
                f"ConfluenceApiToken._encode_auth: Using Bearer+credentials encoding for auth_type='{auth_type}'"
            )
        else:
